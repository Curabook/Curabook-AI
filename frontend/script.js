/**
 * script.js — Curabook PHI  v3.3 — Personalization Update
 * ══════════════════════════════════════════════════════════════
 * CHANGES IN THIS VERSION:
 *
 * PERSONALIZATION:
 *   #P-1  handleLoginSuccess extracts first_name from Supabase user_metadata
 *         and computes a time-of-day greeting:
 *           "Good morning, Aarav" / "Good afternoon, Priya" / "Good evening, …"
 *         Used in the pulse card header and welcome hero.
 *
 *   #P-2  Chat input placeholder becomes context-aware:
 *           - With health data + name:  "Ask about your LDL trend, Aarav…"
 *           - With health data, no name: "Ask about your LDL trend…"
 *           - No data: "Ask PHI about your health…"
 *
 *   #P-3  Profile modal now fetches /api/profile and renders age, gender,
 *         plan, and name so the user can see exactly what PHI knows about them.
 *
 * RELIABILITY:
 *   - All fetch calls use safeJson() — no more "Unexpected end of input"
 *   - All redirects use clean URLs (/login, /signup) via vercel.json
 * ══════════════════════════════════════════════════════════════
 */
...
let currentUserName      = "";    // #P-1: first name extracted from metadata
...
/**
 * #P-1: Extract first_name from user_metadata.
 * Supabase stores it under user.user_metadata.first_name (set during signup).
 * Falls back to the part before @ in the email if not set.
 */
function _extractFirstName(user) {
    const meta = user.user_metadata || {};
    if (meta.first_name) return meta.first_name;
    if (meta.name)       return meta.name.split(" ")[0];
    // Fallback: capitalise the part before @ in email
...
async function handleLoginSuccess(user) {
    currentUser     = user;
    currentUserName = _extractFirstName(user);  // #P-1
    _docCtx.clear();
    _cache.clear();
    ...
    // #P-1: personalized welcome toast
...
function showNoDataState() {
    ...
    // #P-1: personalized hero title on welcome screen
    if (DOM.welcomeHero) {
        const titleEl = DOM.welcomeHero.querySelector(".welcome-title");
        if (titleEl && currentUserName) {
            titleEl.innerHTML = `${currentUserName}'s health <em>co-pilot</em>`;
        }
    }
...
/**
 * #P-1: Pulse card headline now starts with the time-of-day greeting when name is known.
 */
...
/**
 * #P-2: Input placeholder includes name + top trend marker.
 */
...
function sendChip(text) { DOM.userInput.value=text; DOM.userInput.focus(); setTimeout(handleSend,80); }
...
async function renderPulseModalContent() {
    ...
    try {
        ...
        if (!mArr.length) { DOM.pulseModalBody.innerHTML='<div class="loading-text">No health data yet — upload a report.</div>'; return; }
        const rows = mArr.map(m => {
            ...
            return `<div style="display:flex;justify-content:space-between;align-items:center;padding:9px 0;border-bottom:1px solid var(--border)">
                <div><div style="font-weight:500;font-size:13px">${escapeHtml(m.marker_name)}</div>
                <div style="font-size:11.5px;color:var(--text-muted)">Normal: ${escapeHtml(m.reference_range||"—")} · ${escapeHtml(m.date||"")}</div></div>
                <div style="text-align:right"><div style="font-weight:700;font-size:14px;color:${color}">${m.value} <span style="font-size:11px;font-weight:400">${escapeHtml(m.unit||"")}</span></div>
                <div style="font-size:10.5px;font-weight:700;color:${color}">${badge}</div></div></div>`;
        }).join("");
        const insH = iArr.map(ins =>
            `<div style="border-left:3px solid ${ins.severity==="high"?"var(--accent-warn)":ins.severity==="medium"?"var(--accent-amber)":"var(--accent-ok)"};padding:8px 11px;margin-bottom:7px;background:var(--bg-hover);border-radius:0 8px 8px 0">
                ...
                <div style="font-size:12px;color:var(--text-muted);margin-top:2px">${escapeHtml(ins.detail||"")}</div></div>`
        ).join("");
        DOM.pulseModalBody.innerHTML = `
            ${iArr.length?`<div style="margin-bottom:18px"><div style="font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px"><i class="fa-solid fa-lightbulb" style="color:var(--accent-amber)"></i> PHI Synthesis</div>${insH}</div>`:""}
    ...
    } catch { DOM.pulseModalBody.innerHTML='<div class="loading-text" style="color:var(--accent-warn)">Could not load health data.</div>'; }
}
...
function renderHistory(conversations) {
    ...
    DOM.historyList.innerHTML = conversations.map(c =>
        `<div class="history-item${c.id===activeConvId?" active":""}" data-id="${c.id}">
            ...
            <button class="delete-chat" title="Delete" onclick="event.stopPropagation();showDeleteModal('${c.id}')"><i class="fa-solid fa-trash"></i></button>
        </div>`
    ).join("");
}
...
async function openConversation(id) {
    ...
    setProcessing(true);
    activeConvId=id; uploadedFiles=[]; updateFilePreview(); _docCtx.clear(); showChatMode();
    DOM.chatDisplay.innerHTML="";
    document.querySelectorAll(".history-item").forEach(el => el.classList.toggle("active", el.getAttribute("data-id")===id));
    try {
        const h   = await getAuthHeaders();
        ...
        } else { DOM.chatDisplay.innerHTML='<div class="empty-state">No messages yet.</div>'; }
        closeSidebar();
...
async function handleSend() {
    ...
    if (uploadedFiles.length>0) {
        ...
        uploadedFiles=[]; updateFilePreview(); setProcessing(false);
        _cache.del("dashboard"); setTimeout(loadHealthPulse,3000);
    }

    appendMessage(text,"user");
    const botRow=appendTyping(); setProcessing(true);
    ...
    try {
        ...
        const res = await fetch(`${API_BASE}/chat`, {
            method:"POST", headers:h,
            body:JSON.stringify({
                conversation_id:activeConvId, message:text,
    ...
    if (msgCount<=2&&activeConvId) {
        renameConversation(activeConvId, documentContents.length?`📄 ${documentContents[0].name}`:text.substring(0,45));
        loadHistory();
    }
}
...
function appendMessage(text, role) {
    const wrap=document.createElement("div"); wrap.className=`chat-message ${role==="user"?"user-msg":"bot-msg"}`;
    const av=document.createElement("div"); av.className=`msg-avatar ${role==="user"?"user-av":"ai-av"}`;
    ...
    if (role==="user"){wrap.appendChild(bub);wrap.appendChild(av);}else{wrap.appendChild(av);wrap.appendChild(bub);}
    DOM.chatDisplay.appendChild(wrap); DOM.chatDisplay.scrollTop=DOM.chatDisplay.scrollHeight;
    return wrap;
}

function appendTyping() {
    const wrap=document.createElement("div"); wrap.className="chat-message bot-msg";
...
function updateMessage(wrap, text) {
    const b=wrap.querySelector(".msg-bubble"); if (b) b.innerHTML=renderMarkdown(text);
    DOM.chatDisplay.scrollTop=DOM.chatDisplay.scrollHeight;
}

function showChatMode()    { if (DOM.welcomeScreen){DOM.welcomeScreen.style.display="none";DOM.welcomeScreen.classList.add("hidden-welcome");} }
function showWelcomeMode() { if (DOM.welcomeScreen){DOM.welcomeScreen.style.display="";DOM.welcomeScreen.classList.remove("hidden-welcome");} if (_healthContext) renderPulseCard(_healthContext); }
...
async function processFile(file) {
    ...
    try {
        ...
        return {name:file.name,summary:d.summary_text||"",document_id:d.document_id||"",document_text:d.document_text||"",markers:d.markers||[],abnormal:d.abnormal_count||0};
...
function updateFilePreview() {
    ...
    DOM.filePreview.innerHTML=uploadedFiles.map((f,i)=>{
        ...
        return `<div class="file-chip"><i class="fa-solid ${icon}"></i><span>${escapeHtml(f.name)}</span><button class="remove-file" onclick="removeFile(${i})"><i class="fa-solid fa-xmark"></i></button></div>`;
    }).join("");
...
window.closeModals = () => { [DOM.profileModal,DOM.settingsModal,DOM.deleteModal,DOM.pulseModal].forEach(m=>m?.classList.add("hidden")); };
[DOM.profileModal,DOM.settingsModal,DOM.deleteModal,DOM.pulseModal].forEach(m=>m?.addEventListener("click",e=>{if(e.target===m)window.closeModals();}));

function showDeleteModal(id){conversationToDelete=id;DOM.deleteModal?.classList.remove("hidden");}
window.showDeleteModal=showDeleteModal;

/* ── #P-3: Profile — fetches /api/profile for demographics ─── */
...
async function loadProfileStats(user) {
    ...
    const [histRes,dashRes,profileRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/api/profile`,  {headers:h}).catch(()=>null),   // #P-3
    ]);
    ...
    // #P-3: render biographical profile data the AI uses
    if (profileRes?.ok) {
...
/**
 * #P-3: Render age, gender, plan into the profile modal so users can see
 * exactly what PHI knows about them.
 */
function _renderProfileDemographics(profile) {
    // Find or create demographics section in profile modal
    ...
    if (profile.age) {
        rows.push(`<div class="setting-item"><div class="setting-info"><h4><i class="fa-solid fa-cake-candles"></i> Age</h4><p>${escapeHtml(String(profile.age))} years old</p></div></div>`);
    }
    if (profile.gender) {
        const g = String(profile.gender).charAt(0).toUpperCase() + String(profile.gender).slice(1);
        rows.push(`<div class="setting-item"><div class="setting-info"><h4><i class="fa-solid fa-person"></i> Gender</h4><p>${escapeHtml(g)}</p></div></div>`);
    }
    if (profile.plan) {
        const planLabel = profile.plan === "pro" ? "✦ PHI Pro" : "PHI Free";
        rows.push(`<div class="setting-item"><div class="setting-info"><h4><i class="fa-solid fa-star"></i> Plan</h4><p>${escapeHtml(planLabel)}</p></div></div>`);
    }
    ...
    if (rows.length) {
        demoSection.innerHTML = `
            ...
            ${rows.join("")}
            <p style="font-size:11.5px;color:var(--text-muted);margin-top:6px;margin-bottom:4px">
                PHI uses your age and gender to apply clinically appropriate reference ranges.
            </p>
            <hr>`;
    }
}
...
async function loadHealthDashboard() {
    ...
    const h=await getAuthHeaders();
    const [mr,ir]=await Promise.all([fetch(`${API_BASE}/api/health-markers`,{headers:h}).catch(()=>null),fetch(`${API_BASE}/api/health-insights`,{headers:h}).catch(()=>null)]);
...
function renderHealthDashboard(markers,insights) {
    if (!markers.length&&!insights.length){DOM.healthDash.innerHTML='<div class="loading-text">No health data yet. Upload a lab report.</div>';return;}
    const color=s=>s==="HIGH"||s==="LOW"?"var(--accent-warn)":s==="NORMAL"?"var(--accent-ok)":"var(--text-muted)";
    const cards=markers.map(m=>`<div style="background:var(--bg-hover);border-radius:8px;padding:9px;min-width:100px;flex:1"><div style="font-size:10.5px;color:var(--text-muted)">${escapeHtml(m.marker_name)}</div><div style="font-size:1rem;font-weight:700;color:${color(m.status)}">${m.value} <span style="font-size:10px;font-weight:400">${escapeHtml(m.unit||"")}</span></div><div style="font-size:9.5px;color:var(--text-muted)">${escapeHtml(m.date||"")}</div></div>`).join("");
    const insH=insights.map(ins=>`<div style="border-left:3px solid ${ins.severity==="high"?"var(--accent-warn)":ins.severity==="medium"?"var(--accent-amber)":"var(--accent-ok)"};padding:6px 10px;margin-bottom:5px;background:var(--bg-hover);border-radius:0 6px 6px 0"><div style="font-weight:600;font-size:12px">${escapeHtml(ins.headline||"")}</div><div style="font-size:11px;color:var(--text-muted)">${escapeHtml(ins.detail||"")}</div></div>`).join("");
    DOM.healthDash.innerHTML=(cards?`<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:11px">${cards}</div>`:"")+(insH?`<div style="font-size:11px;font-weight:600;color:var(--text-muted);margin-bottom:5px"><i class="fa-solid fa-lightbulb" style="color:var(--accent-amber)"></i> PHI Insights</div>${insH}`:"");
...
function showDoctorBriefModal(){
    ...
    const m=document.createElement("div"); m.id="doctor-brief-modal"; m.className="modal"; m.style.zIndex="10001";
    m.innerHTML=`<div class="modal-box"><div class="modal-header"><h3><i class="fa-solid fa-stethoscope"></i> Doctor Visit Prep</h3><button class="close-btn" onclick="$id('doctor-brief-modal').remove()"><i class="fa-solid fa-xmark"></i></button></div>
    <div style="padding:4px 0 16px"><p style="font-size:13px;color:var(--text-muted);margin-bottom:14px">Personalised brief based on your health memory.</p>
    ...
    <div id="brief-output" style="display:none;margin-top:12px;background:var(--bg-hover);border-radius:8px;padding:13px;font-size:13px;line-height:1.65;max-height:260px;overflow-y:auto"></div></div>
    <div class="modal-actions"><button class="btn-cancel" onclick="$id('doctor-brief-modal').remove()">Cancel</button>
    <button id="genBriefBtn" style="padding:8px 16px;background:var(--brand);border:none;border-radius:8px;color:white;font-size:13.5px;font-weight:600;font-family:var(--font);cursor:pointer"><i class="fa-solid fa-wand-magic-sparkles"></i> Generate</button></div></div>`;
    document.body.appendChild(m);
    $id("genBriefBtn").onclick=async()=>{
        const btn=$id("genBriefBtn"); btn.disabled=true; btn.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';
        try{const h=await getAuthHeaders();const res=await fetch(`${API_BASE}/api/doctor-brief`,{method:"POST",headers:h,body:JSON.stringify({symptoms:($id("brief-symptoms")?.value||"").split(",").map(s=>s.trim()).filter(Boolean),medications:($id("brief-meds")?.value||"").split(",").map(s=>s.trim()).filter(Boolean)})});const d=await safeJson(res);const out=$id("brief-output");if(out){out.style.display="block";out.innerHTML=renderMarkdown(d.brief||"Could not generate. Try again.");}}catch{showToast("Failed to generate brief.","error");}
        btn.disabled=false;btn.innerHTML='<i class="fa-solid fa-wand-magic-sparkles"></i> Generate';
    };
}

async function loadPlanStatus(){const h=await getAuthHeaders();if(!h.Authorization)return;const res=await fetch(`${API_BASE}/api/payment/status`,{headers:h}).catch(()=>null);if(!res?.ok)return;const d=await safeJson(res);if(DOM.dropdownPlan){DOM.dropdownPlan.textContent=d.is_pro?"✦ PHI Pro":"PHI Free";if(d.is_pro)DOM.dropdownPlan.style.color="var(--brand)";}}

function initVoiceInput(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){if(DOM.micBtn){DOM.micBtn.disabled=true;DOM.micBtn.style.opacity="0.3";}return;}let listening=false,rec=null;DOM.micBtn?.addEventListener("click",()=>{if(listening){rec?.stop();return;}rec=new SR();rec.lang="en-US";rec.interimResults=false;rec.maxAlternatives=1;rec.onstart=()=>{listening=true;DOM.micBtn.classList.add("listening");showToast("Listening…");};rec.onresult=e=>{DOM.userInput.value=e.results[0][0].transcript;autoGrow(DOM.userInput);DOM.userInput.focus();};rec.onerror=e=>{const m={"no-speech":"No speech.","not-allowed":"Mic blocked.","network":"Network error."};showToast(m[e.error]||`Error: ${e.error}`,"error");};rec.onend=()=>{listening=false;DOM.micBtn.classList.remove("listening");};try{rec.start();}catch{showToast("Voice failed.","error");}});}

function loadUserPreferences(){const p=JSON.parse(localStorage.getItem("phi_prefs")||"{}");const dark=p.theme==="dark";if(dark)document.body.classList.add("dark-mode");if(DOM.themeToggle)DOM.themeToggle.checked=dark;const fs=p.fontSize||15;document.documentElement.style.setProperty("--chat-font-size",fs+"px");if(DOM.fontSizeInput)DOM.fontSizeInput.value=fs;if(DOM.fontSizeValue)DOM.fontSizeValue.textContent=fs+"px";}
...
function initSettings(){
    ...
    DOM.exportChatBtn?.addEventListener("click",async()=>{if(!activeConvId){showToast("No active conversation.","error");return;}const h=await getAuthHeaders();const res=await fetch(`${API_BASE}/conversation`,{method:"POST",headers:h,body:JSON.stringify({conversation_id:activeConvId})});const d=await safeJson(res);if(!Array.isArray(d)){showToast("Could not export.","error");return;}let out="Curabook PHI Chat Export\n"+"=".repeat(40)+"\n\n";d.forEach(m=>{out+=`${m.role.toUpperCase()}:\n${m.content}\n\n`;});Object.assign(document.createElement("a"),{href:URL.createObjectURL(new Blob([out],{type:"text/plain"})),download:`phi-chat-${Date.now()}.txt`}).click();showToast("Chat exported");});
    DOM.clearHistBtn?.addEventListener("click",async()=>{if(!confirm("⚠️ Delete ALL conversations?"))return;const h=await getAuthHeaders();const res=await fetch(`${API_BASE}/history`,{method:"POST",headers:h});const convs=await safeJson(res);if(Array.isArray(convs))await Promise.all(convs.map(c=>fetch(`${API_BASE}/delete`,{method:"POST",headers:h,body:JSON.stringify({conversation_id:c.id})}).catch(()=>{})));DOM.chatDisplay.innerHTML="";activeConvId=null;uploadedFiles=[];_docCtx.clear();_cache.clear();updateFilePreview();showWelcomeMode();loadHistory();window.closeModals();showToast("All conversations deleted");});
    DOM.exportDataBtn?.addEventListener("click",async()=>{const h=await getAuthHeaders();try{const res=await fetch(`${API_BASE}/export-data`,{method:"POST",headers:h});const d=await safeJson(res);Object.assign(document.createElement("a"),{href:URL.createObjectURL(new Blob([JSON.stringify(d,null,2)],{type:"application/json"})),download:`phi-data-${Date.now()}.json`}).click();showToast("✅ Data exported");}catch{showToast("Export failed.","error");}});
    DOM.deleteAccBtn?.addEventListener("click",async()=>{if(!confirm("⚠️ PERMANENTLY DELETE YOUR ACCOUNT?\n\nAll health data will be erased. Cannot be undone."))return;if(prompt('Type "DELETE" to confirm:')!=="DELETE")return;const h=await getAuthHeaders();try{await supabaseClient.auth.signOut();}catch{}try{const res=await fetch(`${API_BASE}/delete-account`,{method:"POST",headers:h});const d=await safeJson(res);showToast(d.success?"Account deleted.":"Signed out. Contact support if data persists.",d.success?"success":"error");}catch{showToast("Error. Contact support@curabook.com.","error");}setTimeout(()=>window.location.href="/login",1500);});
}
...
function wireEvents(){
    ...
    DOM.newChatBtn?.addEventListener("click",()=>{if(hasChatMessages()&&!confirm("Start a new chat?"))return;activeConvId=null;uploadedFiles=[];_docCtx.clear();DOM.chatDisplay.innerHTML="";updateFilePreview();showWelcomeMode();document.querySelectorAll(".history-item").forEach(el=>el.classList.remove("active"));DOM.userInput.focus();});
    ...
    DOM.fileInput?.addEventListener("change",e=>{Array.from(e.target.files||[]).forEach(f=>{if(f.size>10*1024*1024){showToast(`${f.name} too large. Max 10MB.`,"error");return;}if(!/\.(pdf|txt)$/i.test(f.name)){showToast(`${f.name}: PDF or TXT only.`,"error");return;}uploadedFiles.push(f);});updateFilePreview();DOM.fileInput.value="";if(uploadedFiles.length){showToast(`🔒 ${uploadedFiles.length} file(s) ready`);DOM.userInput.focus();}});
    ...
    document.addEventListener("keydown",e=>{if((e.ctrlKey||e.metaKey)&&e.key==="k"){e.preventDefault();DOM.newChatBtn?.click();}if(e.key==="Escape")window.closeModals();});
}
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (user.app_metadata?.provider==="google"&&!localStorage.getItem(`phi_terms_${user.id}`)) { window.location.href="/login"; return; }

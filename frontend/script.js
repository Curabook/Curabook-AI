 ...
 * The central brain for the frontend application.
 */
"use strict";
    ...
    try {
        ...
        if (!markers.length) {
            content.innerHTML = `
                <div class="hv-empty">
                    ...
                    <br>
                    <button class="hv-cta-btn" onclick="el('fileInput').click()">
        ...
        content.querySelectorAll("[data-ask]").forEach(btn =>
            btn.addEventListener("click", () => {
                switchView("chat");
                setTimeout(() => sendMessage(btn.dataset.ask), 100);
            })
        );
    } catch (e) {
...
function buildHealthViewHTML(markers, dashboard) {
    ...
    if (cliffalerts.length) {
        html += `
        <div class="hv-section">
            <div class="hv-heading"><i class="fa-solid fa-bolt"></i>Active Alerts</div>
    ...
    if (markers.length) {
        html += `
        <div class="hv-section">
            <div class="hv-heading"><i class="fa-solid fa-flask"></i>Lab Values</div>
    ...
    if (abnormal.length) {
        html += `
        <div class="hv-section">
            <div class="hv-heading"><i class="fa-solid fa-circle-exclamation"></i>Needs Attention</div>
            <div class="alert-feed">`;
            ...
            html += `
                <div class="alert-item warn">
                    ...
                    <div class="alert-desc">
                        ${m.value} ${esc(m.unit || "")} — ${m.status === "HIGH" ? "above" : "below"} normal
                        ${m.reference_range ? ` (${esc(m.reference_range)})` : ""}
                    </div>
...
function askAboutReport(f) {
    switchView("chat");
    setTimeout(() => sendMessage(`Summarize my ${f} report and flag any cliff signals.`), 100);
}
...
async function loadHistory() {
    ...
    try {
        ...
        if (list) list.innerHTML = '<div class="sb-empty">Failed to load history.</div>';
    }
}
...
function renderHistory(convs) {
    ...
    groups.forEach((arr, label) => {
        ...
        arr.forEach(c => {
            ...
            html += `
            <div class="hist-item${active}" data-id="${esc(c.id)}">
...
async function openConversation(id) {
    ...
    try {
        ...
        if (ok && Array.isArray(data)) {
            data.forEach(m => appendMsg(m.content, m.role === "user" ? "user" : "ai"));
            scrollBottom();
        }
    } catch {}
}
...
function prependHistory(id, title) {
    ...
    item.innerHTML = `
        <span class="hist-title">${esc(title)}</span>
        <button class="hist-del" data-del="${esc(id)}" title="Delete"><i class="fa-solid fa-trash"></i></button>
    `;
...
async function handleSend() {
    ...
    if (!text && _uploads.length) {
        text = "Please analyze my uploaded document.";
    }
    if (!text) return;
...
async function sendMessage(text) {
    ...
    try {
        ...
        if (_uploads.length) {
            const lr = appendTyping();
            updateTyping(lr, "📄 Processing file...");
        ...
        if (res.ok && data?.reply) {
            ...
            // DYNAMIC SHIELD UPDATE: Auto refresh dashboard elements after the AI processes data
...
function setSendingState(on) {
    ...
    if (btn) {
        btn.disabled = on;
        btn.innerHTML = on ? '<i class="fa-solid fa-spinner fa-spin"></i>' : '<i class="fa-solid fa-arrow-up"></i>';
    }
...
async function processUpload(file) {
    ...
    try {
        let res = await Promise.race([
            doUp(s.access_token),
            new Promise((_, r) => setTimeout(() => r(new Error("timed out")), 65000))
        ]);
...
function renderMarkers(markers) {
    ...
    if (!markers.length) {
        g.innerHTML = '<div class="markers-empty">Upload a lab report to see your markers</div>';
        return;
    }
    ...
    g.innerHTML = markers.slice(0, 10).map(m => {
        ...
        <div class="marker-card">
            ...
            <div class="marker-card-val ${cls}">
                ${m.value}<span class="marker-card-unit"> ${esc(m.unit || "")}</span>
            </div>
            ${badge}
        </div>`;
    }).join("");
...
function runCliffDetection(markers) {
    ...
    if (gk) {
        ...
        if (r.length >= 2) {
            ...
            if (pct >= 15) {
                alerts.push({ type: "danger", title: `🚨 Glucose rebound +${pct.toFixed(0)}%`, desc: `${r[0]._v} → ${r[r.length-1]._v} mg/dL` });
            } else if (pct >= 10) {
                alerts.push({ type: "warn", title: `⚠ Glucose rising +${pct.toFixed(0)}%`, desc: "Approaching 15% threshold." });
            }
        }
    }
    ...
    if (hk) {
        ...
        for (let i = 1; i < r.length; i++) {
            ...
            if (d >= 0.25) {
                alerts.push({ type: "danger", title: `🚨 HbA1c rebound +${d.toFixed(2)}%`, desc: `${r[i-1]._v}% → ${r[i]._v}%` });
                break;
            }
        }
    }

    markers.filter(m => m.status === "HIGH" && !/glucose/i.test(m.marker_name)).slice(0, 2).forEach(m => {
        alerts.push({ type: "warn", title: `⬆ ${m.marker_name} HIGH`, desc: `${m.value} ${m.unit || ""}` });
    });

    if (!alerts.length) {
        alerts.push({ type: "ok", title: "✅ No rebound signals", desc: "All markers stable. Keep up protein + training." });
    }
...
function setBarPct(id, pct) {
    const b = el(id);
    if (b) b.style.width = Math.max(0, pct) + "%";
}
...
function calcProteinDisplay(gw, showDetails = true) {
    if (!gw || gw < 80 || gw > 400) {
        if (showDetails) toast("Enter a valid goal weight (80–400 lbs).", "err");
        return;
    }
    ...
    if (showDetails) {
        ...
        if (d) {
            ...
            d.innerHTML = `
                <strong>${pm}g per meal</strong> across 3 meals — ${lu ? "✅" : "⚠️"} ${lu ? "Meets" : "Below"} 30g leucine threshold<br>
...
const NOISE_MSG = {
    ...
    5: "Moderate.",
    6: "Elevated.",
    7: "High — taper may have been too fast.",
    8: "Very high — biology, not willpower.",
    9: "Intense. Discuss urgently.",
    10: "🚨 Maximum. Provider conversation needed."
};
...
function updateNoiseReadout() {
    ...
    if (r) {
        r.innerHTML = `<strong style="color:${colors[v]}">Level ${v}/10</strong> — ${NOISE_MSG[v]}`;
    }
}
...
async function logNoiseLevel() {
    ...
    await apiFetch("/api/behavioral-logs", {
        method: "POST", headers: h, body: JSON.stringify({ date: new Date().toISOString().slice(0, 10), metric_name: "food_noise", value: val, unit: "1-10" })
    }).catch(() => {});

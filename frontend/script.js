...
/* ═══ BULLETPROOF UTILS (Moved to top to fix setIcon error) ═══ */
function el(id) { return document.getElementById(id); }
function esc(s) { return String(s || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;"); }
function setText(id, v) { const e = el(id); if (e) e.textContent = v; }
function setIcon(id, c) { const e = el(id); if (e) e.className = `fa-solid ${c}`; }
...
async function doSignOut() {
  ...
  try { if (_sb) _sb.auth.signOut({ scope: "global" }).catch(() => {}); } catch(e) {}
...
async function boot() {
  ...
  try {
    ...
    if (session?.user) {
      // THE GHOST SESSION FIX: Check server to make sure token is alive.
      const { data: userData, error } = await _sb.auth.getUser();
      if (error || !userData?.user) {
          console.warn("[PHI] Dead ghost session detected. Clearing state.");
          await _sb.auth.signOut();
          if (!IS_LOCAL) window.location.replace("/login");
          return;
      }
      if (!_initialized) await onSignIn(userData.user);
    } else {
      if (!IS_LOCAL) window.location.replace("/login");
    }
  } catch(err) {

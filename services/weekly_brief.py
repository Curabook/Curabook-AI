...
Generates a personalized 5-line weekly brief for each user and
delivers it via email. This is the primary retention mechanism.

Research basis: Users who interact 25+ times/month have 60% higher
likelihood of metabolic syndrome reversal. The Weekly Brief engineers
those interactions by giving users a reason to return every week.

Brief structure (5 lines, always):
  1. How your tracked markers compare to last week
  2. One pattern PHI noticed
  3. One specific action for this week
  4. One question to ask your doctor at your next visit
  5. One encouraging data point

Delivery:
  - Generated every Monday at 8am user local time
  - Sent via Supabase Edge Functions or any SMTP provider
  - Also stored in DB so users can read it in-app
  - Never sent if user has no health data

Pre-appointment Prep:
  - Triggered when user enters a doctor appointment date
  - Generates a 1-page clinical summary 48 hours before
═══════════════════════════════════════════════════════════════════════════
"""
...
def generate_weekly_brief(
    ...
    """
    Generate a personalized weekly health brief.
    
    Returns dict with:
      {
        "subject":     str,   # email subject line
        "headline":    str,   # 1 sentence
        "pattern":     str,   # PHI noticed something
        "action":      str,   # one thing to do this week
        "doctor_q":    str,   # one question for next appointment
        "win":         str,   # one encouraging data point
        "full_text":   str,   # formatted for email
        "generated_at": str,
      }
    
    Returns None if user has insufficient data.
    """
    # Check if brief already sent this week
    ...
    from health_memory.memory import (
        ...
        get_conversation_memories, build_health_context_block
    )
...
def _build_brief_content(
    ...
    """Build brief using rule-based logic (works without LLM)."""
    abnormal = {
    ...
    improving_trends = [
        t for t in trends
        if not t["concerning"] and t["pct_change"] >= 10
    ]
    worsening_trends = [
        t for t in trends
        if t["concerning"]
...
def _detect_weekly_pattern(latest: dict, trends: list, memories: list) -> str:
    """Find the most interesting pattern in the data."""
    # Check insulin resistance triad
    ...
    if ldl and crp and ldl.get("status") == "HIGH" and crp.get("status") == "HIGH":
        return (
            ...
            f"is a cardiovascular risk pattern — inflammation combined with high cholesterol is more "
            f"concerning than either alone. This is the combination to lead with at your next appointment."
        )
    ...
    return (
        f"PHI noticed: you have {len(latest)} markers being tracked across "
        f"{len(set(v.get('source_document','') for v in latest.values() if v.get('source_document')))} "
        f"reports. The more data PHI has over time, the more specific the patterns become."
    )


def _suggest_weekly_action(abnormal: dict, worsening: list, memories: list) -> str:
    """One specific, achievable action for this week."""
    # Priority: address most concerning worsening trend
    if worsening:
        t = worsening[0]
        ...
        if "ldl" in marker or "cholesterol" in marker:
            return (
                ...
                f"not as an alarm, but as a proactive question."
            )
        if "hba1c" in marker or "glucose" in marker:
            return (
                f"This week: try logging your meals and glucose readings for just 3 days — "
                f"not to judge, but to give PHI more data to work with. "
                f"A 20-minute post-meal walk on those same days would give your data the most useful signal."
            )
    
    # Address most concerning abnormal
    if abnormal:
        ...
        if "vitamin d" in k.lower():
            return (
                ...
                f"supplementation is a low-effort change with measurable effects within 8–12 weeks."
            )
        if "ferritin" in k.lower() or "hemoglobin" in k.lower():
            return (
                f"This week: ask your provider whether iron testing (including serum iron and TIBC) "
                f"would be appropriate. Your {k} level suggests iron stores worth investigating."
            )
    
    # Default: upload a report if none recently
    return (
        "This week: if you have a recent lab report you haven't uploaded yet, "
        "add it to PHI. The more longitudinal data in your record, the more specific "
        "PHI can be about what's actually changing and why."
    )


def _generate_doctor_question(abnormal: dict, trends: list) -> str:
    """One specific question to ask at the next appointment."""
    if trends:
    ...
    return (
        ...
        "given my current health history?' — this opens a conversation about what PHI "
        "might be missing."
    )
...
def _find_weekly_win(latest: dict, trends: list, memories: list) -> str:
    ...
    # Improving trends are the clearest win
    ...
    # Normal markers as a win
    normal = [k for k, v in latest.items() if v.get("status") == "NORMAL"]
    if len(normal) >= 3:
        return (
            f"{len(normal)} of your tracked markers — including {', '.join(normal[:2])} — "
            f"are within normal range. That's a meaningful baseline to build from."
        )
    
    # Consistency win from memories
    if memories:
        return (
            f"You've been engaged with your health data — PHI has {len(memories)} facts "
            f"from your conversations. That kind of ongoing attention to your own health "
            f"is genuinely uncommon, and it's what makes personalized insights possible."
        )
    
    return (
        "You uploaded health data and asked questions. That active engagement "
        "with your own health is something most people never do — and it's exactly "
...
def _generate_subject(headline: str, name: str) -> str:
    ...
    # Keep subjects under 60 chars for mobile
...
def _format_email(brief: dict, name: str) -> str:
    """Format the brief as plain text email."""
...
To stop receiving these briefs, update your preferences at curabook.com/settings.
"""
...
def _enhance_with_llm(
    ...
    """Use LLM to make the brief more personal and specific."""
    ...
    try:
        if openai_key:
            ...
            resp = Groq(api_key=groq_key).chat.completions.create(
                model="llama-3.3-70b-versatile", messages=prompt,
                temperature=0.4, max_tokens=600,
            )
            raw = resp.choices[0].message.content.strip()
...
def _brief_sent_this_week(supabase, user_id: str) -> bool:
    """Check if a brief was already generated/sent this week."""
    try:
...
def _store_brief(supabase, user_id: str, brief: dict) -> None:
    """Store the brief for in-app reading and audit."""
    try:
...
def generate_preappointment_prep(
    ...
    """
    Generate a complete pre-appointment brief.
    Called 48 hours before a scheduled appointment.
    
    Returns a structured brief the user can show their doctor.
    """
    ...
    if worsening:
        ...
        lead = f"All tracked markers are within normal range — a useful baseline to document."
    
    # Build 3 specific questions
    questions = _build_appointment_questions(abnormal, worsening, trends, specialist_type, memories)
    
    # Don't forget list from memories
    dont_forget = [m for m in memories[:5] if any(
        kw in m.lower() for kw in ["symptom", "feel", "pain", "tired", "medication", "started"]
    )]
...
def _build_appointment_questions(abnormal, worsening, trends, specialist, memories) -> list[str]:
    questions = []
    ...
    # Insurance / GLP-1 question from memories
...
def _suggest_next_tests(abnormal: dict, trends: list, specialist: str) -> list[str]:
    suggestions = []
    if any("hba1c" in k.lower() for k in abnormal):
        suggestions.append("Fasting insulin — not in standard panel, but critical for insulin resistance picture")
    if any("ldl" in k.lower() for k in abnormal):
        suggestions.append("ApoB (apolipoprotein B) — more accurate cardiovascular risk marker than LDL alone")
